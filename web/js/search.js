...
var makeAlbumTrack = function(t,el,a){
	...
	artist = a.compilation && artist ? " - "+ artist: "";
	el.find(".albumTrackTitle").text(t.title + artist);
	el.find(".albumTrackPosition").text(t.tracknum);
	el.find(".albumTrackDuration").text(toMinSec(t.duration));
...
var albumSelector = function(t,a){
	t.click(function(){
		if ($(".albumViewer").is(":visible")){return;}
		var p = t.offset();
		...
		var hide = big.find(".albumResultAlbum, .albumResultArtist");
		hide.addClass("faded");
		...
		fetchCover(im,tracklist[0].coverid+"_500x500.png", function(){
			setTimeout(function(){
				...
				big.find("div.albumResultCover").css({width:"30%", height:"auto"});
				big.css({left:"1%",top:"45px", width:"97%",height:"90%"});
...
var showAlbums = function(){
	var i=albumIndex;
	for (i; i<Math.min(results.albums.length,100 + albumIndex); i++){
		var a = results.albums[i]
		var t = albumTemplate.clone();
		t.attr("id","albumId"+a.id);
		var im = t.find("img");
		fetchCover(im,Library.getTracks(a.id)[0].coverid+"_150x150.png")
		t.find(".albumResultAlbum").text(a.album);
		t.find(".albumResultArtist").text(a.compilation ? "Various Artists" : Library.getArtist(a.artist_id).artist);
...
$("#searchLibrary").keyup(function(e){
	...
	searchTimer = setTimeout(function(){
		...
		/*$("#albumsView").text("Albums - " + results.albums.length);
		$("#artistsView").text("Artists - " + results.artists.length);
		$("#titlesView").text("Titles - " + results.titles.length);*/
		...
		(function(){
			var o = {}, i, l = results.albums.length,r = [];
			for (i = 0; i < l; i =i+1) {
